...
def render(p, P, screen_size, screen_res):
    ...
    screen_size = np.array([screen_size.width, screen_size.height])
    screen_res = np.array([screen_res.width, screen_res.height])
...
if __name__ == "__main__":
    ...
    fullscreen = True
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, config.cam_res.width)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, config.cam_res.height)

    cube_size = np.array([config.screen_size.width, config.screen_size.height, config.screen_size.height])
    test_points = np.concatenate([transform(box(), transform_matrix(s=cube_size,
                                                                    t=[0., 0., -config.screen_size.height * 0.5])),
                                  transform(box(), transform_matrix(s=cube_size,
                                                                    t=[0., 0., -config.screen_size.height * 2.5])),
                                  transform(box(), transform_matrix(s=cube_size,
                                                                    t=[0., 0., -config.screen_size.height * 4.5]))])
    ...
    while cap.isOpened():
        ...
        image_out = np.zeros((config.screen_res.height, config.screen_res.width, 3), np.uint8)
        if not success:
            continue
        texts = []
        ...
        if result is not None:
            ...
            resized_image = cv2.resize(image, [config.cam_res.width//2, config.cam_res.height//2])
